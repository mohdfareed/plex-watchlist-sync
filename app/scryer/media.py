...
from .models import MediaRequest, Title, _Model
...
class _TitlePage(_Model):
    items: list[Title]
    has_more: bool


class _TitlesResponse(_Model):
    titles: _TitlePage


class _RequestsResponse(_Model):
    media_requests: list[MediaRequest]


class _VersionResponse(_Model):
    scryer_version: str
...
def list_titles(client: ScryerClient, *, stop: Event) -> list[Title]:
    ...
    while True:
        ...
        page = client._query(_TITLES_QUERY, {"offset": offset}, _TitlesResponse).titles
...
def list_media_requests(client: ScryerClient) -> list[MediaRequest]:
    ...
    """
    return client._query(_REQUESTS_QUERY, {}, _RequestsResponse).media_requests


def get_version(client: ScryerClient) -> str:
    """Read the running version for diagnostics, without probing or changing settings."""
    return client._query("query Version { scryerVersion }", {}, _VersionResponse).scryer_version
